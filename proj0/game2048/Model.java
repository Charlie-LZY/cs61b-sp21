...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        for (int col = 0; col < board.size(); col++) {
            int lastTileRow = board.size() - 1;
            for (int row = board.size() - 2; row >= 0; row--) {
                Tile t = board.tile(col, row);
                Tile lastTile = board.tile(col, lastTileRow);
                ...
                if (lastTile == null) {
                    ...
                    changed = true;
                } else if (t.value() == lastTile.value()) {
                    board.move(col, lastTileRow, t);
                    score += lastTile.value() * 2;
                    lastTileRow -= 1;
                    changed = true;
                } else if (row != lastTileRow - 1){
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        for (int col = 0; col < 4; col++) {
            for (int row = 0; row < 4; row++) {
                ...
                for (int i = 0; i < 4; i++) {
                    int new_col = col + directions[i][0];
                    int new_row = row + directions[i][1];
                    if ((new_col >= 0 && new_col < 4) && (new_row >= 0 && new_row < 4)) {
                        if (b.tile(col, row).value() == b.tile(new_col, new_row).value()) {
                            return true;
                        }
                    }
                }
